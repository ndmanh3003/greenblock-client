...
const FrameContent = ({ title, desc, link, index }: IFrameContent) => {
  const type = index % 2
  ...
  return (
    ...
    >
      {type ? (
        ...
        <>
          <img
            src='illustrator/wave-left.svg'
            className='absolute min-h-full left-0'
          />
          <img
            src='illustrator/wave-right.svg'
      ...
      <Link to={link} target='_blank'>
        <ButtonC
          variant='linear'
          className='!text-base !bg-transparent !border-2 border-white !px-8 !py-4 hover:!bg-white hover:!bg-opacity-10 !delay-0'
        >
          Read more
        </ButtonC>
      </Link>
